...
function manageFormFields() {
  ...
  if (isset($_POST['submit_new_form_field'])) {
    ...
    $wpdb->query($wpdb->prepare("INSERT INTO $da_members_form_fields_table (label, priority,field_type) 
                                 VALUES (%s, %s,%s)", $field_label, $field_priority, 'text'));
    $wpdb->query("ALTER TABLE $da_members_members ADD $field_label varchar(50);");
  }
